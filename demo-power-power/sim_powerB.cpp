...
static void generic_simulator()
{
    ...
    ObjectCommInterface *simObjB = Integrator::getCommInterface("simObjB");
    ...
    while (!Integrator::isFinished() && time_current < 10) {
         ...
         * registered earlier */
        {
            cout << "PowerB: Working. Time is " << time_current << endl;

            /* Check for incoming messages. */
            while (simObjB->hasMoreMessages()) {
                Message *incoming_message = simObjB->getNextInboxMessage();
                cout << "PowerB: simObjB received message from "
                    << incoming_message->getFrom()
                    << " with timestamp "
                    << incoming_message->getTime() << endl;
                delete incoming_message;

                /* Send an empty (NULL, size 0) message from simObjB to
                 * simObjA. By default, all messages are 'routed' through
                ...
                simObjB->send(outgoing_message);
                cout << "PowerB: sent message from simObjB to simObjA" << endl;
            }
